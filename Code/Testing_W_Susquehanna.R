...
# Run model
# ports
opt8 = nlminb(start=obj8$env$last.par.best[-c(obj8$env$random)], objective=obj8$fn, gradient=obj8$gr, control=list(eval.max=1e4, iter.max=1e4, trace=1, rel.tol=1e-14) ) # fails
...
SD8.lbfgsb = sdreport( obj8, bias.correct=F ) # fails
...
SD8.bobyqa = sdreport( obj8, bias.correct=F ) # fails
#--------------------------------------------------
...
# Make object
dyn.load( dynlib(paste0("Code/", Version )))
obj7h <- MakeADFun(data=Inputs$Data, parameters=Inputs$Params, random=Inputs$Random, map=Inputs$Map, hessian=FALSE, inner.control=list(maxit=1000) )
Report = obj7h$report()

# First run
obj7h$fn( obj7h$par )
# Check for parameters that don't do anything
Which = which( obj7h$gr( obj7h$par )==0 )

# Run model
# bobyqa - Works
library(optimx)
#lower7h <- c(log(0), log(1e-4), log(0), log(0), 0, log(1e-4), log(1e-4), -100, log(0), -10, -10, -10, -10, log(0.01))
#upper7h <- c(log(7h00), log(100), log(7h00), log(100), 100, log(100), log(100), 100, log(100), 10, 10, 10, 10, log(10))

time1_bobyqa_7h <- Sys.time()
opt7h_bobyqa <- bobyqa(obj7h$env$last.par.best[-c(obj7h$env$random)], fn = obj7h$fn) # optimx(obj7h$par, fn = obj7h$fn, gradient=obj7h$gr, method = "bobyqa", lower = lower7h, upper = upper7h)
SD7h_bobyqa <- sdreport(obj7h) 
time2_bobyqa_7h <- Sys.time()
time_bobyqa_7h <- time2_bobyqa_7h - time1_bobyqa_7h

Report7h_bobyqa = obj7h$report()

data.frame(names(SD7h_bobyqa$value), SD7h_bobyqa$sd)
opt7h_bobyqa[["final_gradient"]] = obj7h$gr( opt7h_bobyqa$par )
opt7h_bobyqa$ierr # 
opt7h_bobyqa[["AIC"]] = 2*opt7h_bobyqa$fval + 2*length(opt7h_bobyqa$par)

# LBFGSB - Fails
time1.lbfgsb.7h <- Sys.time()
opt7h_LBFGSB <- optimx(obj7h$par, fn = obj7h$fn, gradient=obj7h$gr, method = "L-BFGS-B") # fails
SD7h_LBFGSB <- sdreport(obj7h) # 
time2.lbfgsb.7h <- Sys.time()
time.lbfgsb.7h <- time2.lbfgsb.7h - time1.lbfgsb.7h # 

Report7h.lbfgsb = obj7h$report()


data.frame(param = names(SD7h_bobyqa$value), 
           mean7 = SD7_bobyqa$value, 
           mean7h = SD7h_bobyqa$value, 
           SD7 = SD7$sd, 
           SD7h = SD7h_bobyqa$sd)

data.frame(param = c("log_mean", unlist(dimnames(X_ij)[2]), "sigmaIID", "theta", "theta_st", "rhot", "sigmat"),
           mean7 = c(log_mean = Report7_bobyqa$log_mean, Report7_bobyqa$gamma_j, Report7_bobyqa$sigmaIID, Report7_bobyqa$theta, Report7_bobyqa$theta_st, Report7_bobyqa$rhot, Report7_bobyqa$sigmat),
           mean7h = c(log_mean = Report7h_bobyqa$log_mean, Report7h_bobyqa$gamma_j, Report7h_bobyqa$sigmaIID, Report7h_bobyqa$theta, Report7h_bobyqa$theta_st, Report7h_bobyqa$rhot, Report7h_bobyqa$sigmat))
...
# Run model
# ports
opt10 = nlminb(start=obj10$env$last.par.best[-c(obj10$env$random)], objective=obj10$fn, gradient=obj10$gr, control=list(eval.max=1e4, iter.max=1e4, trace=1, rel.tol=1e-14) ) # fails
opt10[["final_gradient"]] = obj10$gr( opt10$par )
opt10[["AIC"]] = 2*opt10$objective + 2*length(opt10$par)
