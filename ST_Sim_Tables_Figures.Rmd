...
# Make the plots
y_N <- ggplot(df_converged, aes(n_years, mean_N_est, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + geom_hline(yintercept = mean(df_converged$mean_N, na.rm = T), linetype = 2, size = 1, colour = "red") + theme(legend.position = "top") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_N_se <- ggplot(df_converged, aes(n_years, N_se, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + theme(legend.position = "none") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_gamma <- ggplot(df_converged, aes(n_years, gamma_j_hat, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + geom_hline(yintercept = df_converged$gamma_j, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_RMSE <- ggplot(df_converged, aes(n_years, RMSE, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + scale_y_continuous(limits = c(5, 40)) + theme(legend.position = "none") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_theta <- ggplot(df_spatial, aes(n_years, theta_hat, group = n_years)) + geom_boxplot(aes(fill = "#619CFF")) + scale_fill_manual(values=c("#619CFF"))  + theme(legend.position="none") + geom_hline(yintercept = df_spatial$theta, linetype = 2, size = 1, colour = "red") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_rhot <- ggplot(df_converged, aes(n_years, rhot_hat, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_y_continuous(limits = c(0.4, 1)) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + geom_hline(yintercept = df_converged$rhot, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_sigmat <- ggplot(df_converged, aes(n_years, sigmat_hat, group = interaction(n_years, spatialTF))) + geom_boxplot(aes(fill = factor(spatialTF))) + scale_y_continuous(limits = c(0, 4)) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + geom_hline(yintercept = df_converged$sigmat, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + xlab("") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_thet_st <- ggplot(df_spatial, aes(n_years, theta_st_hat, group = n_years)) + geom_boxplot(aes(fill = "#619CFF")) + scale_fill_manual(values=c("#619CFF")) + scale_y_continuous(limits = c(0, 5)) + geom_hline(yintercept = df_spatial$theta_st, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + geom_text(x = 5, y = 65, label = "b", size = 10)

y_rho_st <- ggplot(df_spatial, aes(n_years, rho_st_hat, group = n_years)) + geom_boxplot(aes(fill = "#619CFF")) + scale_fill_manual(values=c("#619CFF")) + scale_y_continuous(limits = c(0, 1)) + geom_hline(yintercept = df_spatial$rho_st, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + geom_text(x = 5, y = 65, label = "b", size = 10)
...
```{r add abundance, warning=FALSE, message=FALSE}
# Add abundance data to flowlines
load("Output/Power_Sim/Data/Data/sim_188_st_1_sites_359_years_20.RData") # includes simulated data and model results
...
```{r ggplot group 2, fig.width=8, fig.height=10, fig.align='center'}
...
# Arrange in rows
st_grid <- arrangeGrob(p_true[[2]] + ggtitle("True N: Year 2") + theme(legend.position="none", axis.title.x = element_blank()),
             p_true[[4]] + ggtitle("True N: Year 4") + theme(legend.position="none", axis.title.x = element_blank()),
             p_true[[6]] + ggtitle("True N: Year 6") + theme(legend.position="none", axis.title.x = element_blank()),
             p_true[[8]] + ggtitle("True N: Year 8") + theme(legend.position="none", axis.title.x = element_blank()),
             p_true[[10]] + ggtitle("True N: Year 10") + theme(legend.position="none", axis.title.x = element_blank()),
             my_legend,
             p_hat[[2]] + ggtitle(expression(paste(widehat(N), ": Year 2", ))) + theme(legend.position="none"),
             p_hat[[4]] + ggtitle(expression(paste(widehat(N), ": Year 4", ))) + theme(legend.position="none"),
             p_hat[[6]] + ggtitle(expression(paste(widehat(N), ": Year 6", ))) + theme(legend.position="none"),
             p_hat[[8]] + ggtitle(expression(paste(widehat(N), ": Year 8", ))) + theme(legend.position="none"),
             p_hat[[10]] + ggtitle(expression(paste(widehat(N), ": Year 10", ))) + theme(legend.position="none"),
             my_legend,
             nrow = 2,
             heights = c(3, 3),
             widths = c(4, 4, 4, 4, 4, 1))
             
ggsave(filename = "Output/Power_Sim/ST_Grid.png", plot = st_grid)
ggsave(filename = "Output/Power_Sim/ST_Grid.pdf", plot = st_grid, dpi = 300)
