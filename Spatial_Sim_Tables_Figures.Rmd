...
# Make the plots
y_N <- ggplot(df_converged, aes(theta, mean_N_hat, group = interaction(theta, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod)), outlier.colour = "NA") + scale_fill_manual(values=c("#F8766D", "#619CFF"), name = "Spatial", labels = c("FALSE", "TRUE")) + geom_hline(yintercept = mean(df_converged$mean_N, na.rm = T), linetype = 2, size = 1, colour = "red") + theme(legend.position = "right") + ylim(c(0,75)) + ylab("Mean reach abundance per 100 m") # + stat_summary(fun.y=mean, colour="darkred", geom="point", shape=18, size=3, aes(theta, mean_N, group = interaction(theta, sp_mod)))

y_N_se <- ggplot(df_converged, aes(theta, N_se, group = interaction(theta, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod)), outlier.colour = "NA") + scale_fill_manual(values=c("#F8766D", "#619CFF")) + theme(legend.position = "none") + ylim(c(0, 1.5)) + ylab("SE of mean reach abundance")

y_gamma <- ggplot(df_converged, aes(theta, coef_hat, group = interaction(theta, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod)), outlier.colour = "NA") + scale_fill_manual(values=c("#F8766D", "#619CFF")) + geom_hline(yintercept = df_converged$coef_true, linetype = 2, size = 1, colour = "red") + theme(legend.position = "none") + ylim(c(0.3, 0.75)) + ylab(expression(paste("Fixed effect coefficient (", gamma, ")")))

y_RMSE <- ggplot(df_converged, aes(theta, rmse, group = interaction(theta, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod)), outlier.colour = "NA") + scale_fill_manual(values=c("#F8766D", "#619CFF")) + ylim(c(5, 40)) + theme(legend.position = "none") + ylab("RMSE")

y_theta <- ggplot(df_spatial, aes(theta, theta_hat, group = theta)) + geom_boxplot(aes(fill = "#619CFF"), outlier.colour = "NA") + scale_fill_manual(values=c("#619CFF"))  + theme(legend.position="none") + stat_summary(fun.y=mean, colour="darkred", geom="point", shape=18, size=3, aes(theta, theta, group = theta)) + ylim(c(0, 10)) + ylab(expression(paste("Expected value of ", theta))) #+ geom_text(data = means, aes(label = weight, y = weight + 0.08))
...
# Make the plots
s_N <- ggplot(df_converged, aes(SD_ou, mean_N_hat, group = interaction(SD_ou, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + xlab("") + ylim(c(0, 75)) # + geom_hline(yintercept = mean(df_converged$mean_N, na.rm = T), linetype = 2, size = 1, colour = "red") + theme(legend.position = "top") +

s_N_se <- ggplot(df_converged, aes(SD_ou, N_se, group = interaction(SD_ou, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + theme(legend.position = "none") + xlab("") + ylim(c(0, 1.5))

s_gamma <- ggplot(df_converged, aes(SD_ou, coef_hat, group = interaction(SD_ou, sp_mod))) + geom_boxplot(aes(fill = factor(sp_mod))) + scale_fill_manual(values=c("#F8766D", "#619CFF")) + theme(legend.position = "none") + xlab("") + ylim(c(0.3, 0.7)) #+ geom_hline(yintercept = df_converged$coef, linetype = 2, size = 1, colour = "red")
...
s_sd <- ggplot(df_spatial, aes(SD_ou, SD_ou_hat, group = SD_ou)) + geom_boxplot(aes(fill = "#619CFF")) + scale_fill_manual(values=c("#619CFF"))  + theme(legend.position="none") + xlab("") #+ geom_abline(intercept = 0, slope = 1)#+ stat_summary(fun.y=mean, colour="darkred", geom="point", shape=18, size=3, aes(SD_ou, theta, group = SD_ou)) # + geom_hline(yintercept = df_spatial$theta, linetype = 2, size = 1, colour = "red")
...
# Create grid of plots
library(gridExtra)
grid.arrange(s_gamma, legend, s_N, s_N_se, s_RMSE, s_theta, ncol = 2)
