...
summary_adult <- summary(adult$SD5, "report", p.value = TRUE)
...
summary_yoy <- summary(yoy$SD5, "report", p.value = TRUE)
summary_yoy <- data.frame(Parameter = c(rownames(summary_yoy)[1:(length(rownames(summary_yoy))-length(yoy$Parameters))], yoy$Parameters), summary_yoy, stringsAsFactors = FALSE)
...
ggplot(df_decay_long, aes(Distance, rho_nu, group = Stage)) + geom_line(aes(colour = Stage)) + xlab("Hydrologic distance (km)") + ylab(expression(hat(rho)[nu](S))) + theme_bw_journal()

ggsave(filename = "Output/Figures/Trout_Dist_Correlation.pdf")
