...
dist_mean <- mean(df$dist_b, na.rm = TRUE)
dist_min <- min(df$dist_b, na.rm = TRUE)
dist_max <- max(df$dist_b, na.rm = TRUE)
dist_median <- median(df$dist_b, na.rm = TRUE)
...
data.frame(mu = exp(adult$Report5$log_mean),
           sigma.mu = adult$Report5$sigmaIID,
           theta = NA_real_,
           rhot = adult$Report5$rhot,
           sigmat = adult$Report5$sigmat,
           theta_st = adult$Report5$theta_st,
           SD_st = adult$Report5$SDinput_st,
           # var_sum_sp = adult$Report5$SDinput^2 + adult$Report5$SDinput_st^2,
           # SD_st_inf = adult$Report5$SD_st_inf,
           rho_st = adult$Report5$rho_st,
           gamma_j = adult$Report5$gamma_j, 
